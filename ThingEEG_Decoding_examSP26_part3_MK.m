...
% Part 3: single-trial decoding instead of pseudo-averaging
...
% Part 3: pseudo-averaging removed -> Npseudo / nPseudo_tst no longer needed

% Part 3: report test pool size directly (every test trial is used individually)
fprintf('Train trials/class: %d  |  Test trials/class: %d  (single-trial decoding)\n', ...
    nTrain, nTest);
...
for iterX = 1:nIterations
    ...
    % 5-fold cross-validation for this iteration (replaces hold-out)
    ...
    for t = 1:nTS
        ...
        trainA = double(squeeze(pseudoData(1, idx_train_A, :, ts)));
        trainB = double(squeeze(pseudoData(2, idx_train_B, :, ts)));
        ...
        %% Part 3: single-trial test data (pseudo-averaging removed)
        % every test trial is used individually instead of averaging Npseudo trials
        testA = double(squeeze(pseudoData(1, idx_test_A, :, ts)));
        testB = double(squeeze(pseudoData(2, idx_test_B, :, ts)));

        % Apply same normalization parameters from training
        testX      = [testA; testB];
        testX_norm = (testX - minTr) ./ rng_tr;
        % one label per single test trial (no pseudo-trials anymore)
        testY      = [ones(size(testA,1),1); 2*ones(size(testB,1),1)];
...
for p = 1:nPerms
    ...
    % 5-fold cross-validation (replaces hold-out); accuracy averaged over folds
    n_folds = 5;
    ...
    for fold = 1:n_folds
        ...
        for t = 1:nTS
            ts = timeSamps(t);
            trainA = double(squeeze(permData{1}(idx_tr1, :, ts)));
            trainB = double(squeeze(permData{2}(idx_tr2, :, ts)));
            trainX = [trainA; trainB];
            trainY = [ones(size(trainA,1),1); 2*ones(size(trainB,1),1)];
            % Normalize
            ...
            % Part 3: single-trial test data here as well
            testA = double(squeeze(permData{1}(idx_te1, :, ts)));
            testB = double(squeeze(permData{2}(idx_te2, :, ts)));
            testX      = [testA; testB];
            testX_norm = (testX - minTr) ./ rng_tr;
            % Part 3: one label per single test trial
            testY      = [ones(size(testA,1),1); 2*ones(size(testB,1),1)];
            mdl   = fitcsvm(trainX_norm, trainY, ...
...
% Part 3: figure name flags the single-trial analysis
figure('Name', 'EEG Decoding: Animal vs. Non-Animal (single trials)', ...
...
subjectID = 'S12';
% Part 3: title flags single trials instead of pseudo-averaged
title('Animal vs. Non-Animal  |  ROI: global | Single trials');
...
%% save
% Part 3: separate output filenames so Part 1 results are not overwritten
save('DA_part3.mat','DA');
save('DA_sh_part3.mat','DA_sh');
