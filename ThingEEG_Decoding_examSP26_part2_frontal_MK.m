...
%% Part 2: restrict the analysis to FRONTAL electrodes only
...
% frontal ROI (electrodes taken from the figure)
roiNames = {'AF7','AF3','AFz','AF4','AF8','Fp1','Fp2'}; % GND carries no data -> excluded

% find indices of the ROI electrodes
roiIdx = find(ismember(chanLabels, roiNames));

fprintf('Frontal ROI: %d electrodes used (%s)\n', numel(roiIdx), strjoin(roiNames,', '));
...
subjectID = 'S12';
% Part 2: title shows the frontal ROI instead of "global"
...
save('DA_part2_frontal.mat','DA');
save('DA_sh_part2_frontal.mat','DA_sh');
